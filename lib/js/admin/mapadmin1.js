...
comboPrioritePOI.on('select', function() {
   console.log("Appel getMarker avec priorite = "+comboPrioritePOI.getValue());
	getMarker(tabcheckchild,comboPrioritePOI.getValue());
});
...
function getMarker(listType, priority) {
	clearMarker();
	console.info('Dans mapadmin1.js getMarker '+tabcheckchild);
	if (tabcheckchild != '') {
		console.info('Dans mapadmin1.js getMarker '+tabcheckchild);
		document.getElementById("loader").style.display = "block";
		var uri = 'lib/php/admin/getMarker.php?listType='+tabcheckchild+'&priority='+priority+'&commentToModerate='+displayObservationsWithCommentToModerate;
...
function displayMarker(response) {
	...
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
			...
			//feat.attributes = {"lib": lib, "desc": desc, "prop": prop, "iconCls": iconCls, "id": id, "photo": photo, "num": num, "rue": rue, "commune": commune, "repgt": repgt, "cmt": cmt, "date": date,"lastDateModif":lastDateModif,  "comments":comments};
			feat.attributes = {"id": json.markers[i].id,"lib": json.markers[i].lib, "desc": json.markers[i].desc, "prop": json.markers[i].prop, "iconCls": json.markers[i].iconCls, "photo": json.markers[i].photo, "num": json.markers[i].num, "rue": json.markers[i].rue, "commune": json.markers[i].commune, "repgt": json.markers[i].repgt, "cmt": json.markers[i].cmt, "date": json.markers[i].date,"lastDateModif":json.markers[i].lastdatemodif_poi, "comments": comments,"icon":json.markers[i].icon};
            features.push(feat);
			
			
			
			//createFeatureAdminMap(lonlat.lon,lonlat.lat, json.markers[i].icon, json.markers[i].lib, json.markers[i].desc, json.markers[i].prop, json.markers[i].iconCls, json.markers[i].id, json.markers[i].photo, json.markers[i].num, json.markers[i].rue, json.markers[i].commune, json.markers[i].repgt, json.markers[i].cmt, json.markers[i].date, json.markers[i].lastdatemodif_poi,comments);
		}
...
function createPopup(feature) {
	...
	htmlGeneral = "";
	//htmlGeneral += "<br/><b>"+T_lib+" :</b> "+featureToDisplay.attributes.lib;
	htmlGeneral += "<br/><b>"+T_dateCreation+" :</b> "+featureToDisplay.attributes.date;
	...
	if (featureToDisplay.attributes.repgt != '') {
		htmlGeneral += "<br/><b>"+T_reponseGrandToulouse+" :</b> "+featureToDisplay.attributes.repgt;
	} else {
    ...
    htmlGeneral += '<br/><div style="width:100%;"><a href="'+window.location.href.substring(0, window.location.href.lastIndexOf('/'))+'/admin.php?id='+featureToDisplay.attributes.id+'"><img src="./resources/icon/silk/script_save.png" style="float:right;" /></a></div>';
	}else{
...
function createFeatureAdminMap(X,Y,icon,lib,desc,prop,iconCls,id,photo,num,rue,commune,repgt,cmt,date,lastDateModif,  comments) {
...
treePanel.on('checkchange', function(n) {
	console.debug("checkchange debut");
	...
	treePanel.on('load', function(n) {
		...
		if (this.getChecked('id')!=''){
			...
			getMarker(this.getChecked('id'),comboPrioritePOI.getValue());
...
function getCheckChild(tab) {
	console.debug("getCheckChild debut");
...
var mapAdminPanel = new GeoExt.MapPanel({
	...
	tbar: new Ext.Toolbar({
		height: '20',
		cls: 'x-panel-header',
		items: ['<span style="color:#333; font-family:tahoma,arial,verdana,sans-serif; font-weight:normal">'+T_applicationName+'</span>', comboPrioritePOI,checkboxDisplayObservationsWithCommentToModerate, ' ',searchObservationField,searchObservationButton,'->', previous, '  ', '  ', next, /*' ', '  ', panButton, '  ', ' ', zoomboxButton, ' ', ' ', ' ', ' ', ' ', */' ', ' ', /*checkhillshading, ' ', geoNameSearchCombo,*/comboALL]
	})
});
...
// modification de la config de la carte par défaut
...
////var mapnik3 = mapLayersArray[0][2];//new OpenLayers.Layer.OSM("OpenStreetMap Mapnik", "https://tile.openstreetmap.org/${z}/${x}/${y}.png",
...
//console.debug("MAPADMIN1 " +mapLayersArray[0][2] );
////expandMap2.setBaseLayer(mapnik3);
////expandMap2.addLayer(mapnik3);
//expandMap2.addControl(new OpenLayers.Control.LayerSwitcher());
//expandMap2.addControl(new OpenLayers.Control.Attribution());
////mapLayersArray[0][2]
...
treePanel.on('expandnode', function(node) {
	console.debug("expand node debut");
