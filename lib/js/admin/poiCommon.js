...
//                	comments += '<li><a href="./resources/pictures/'+json.markers[i]['photos'][j]+' target="_blank">Photo associée</a></li>';
//                }
//                comments += '</ul><hr />';
//                j++;
//            }
//            console.info('displayMarkerAndCenterInAdminMap ' + comments);
//
//            } 
	        ...
	        load: function() {
	            ...
	            if (noIdParam != -1) {
	                if (idEdit != '' && idEdit != undefined && alreadyShowExpandWindowGetParameter == 0) {
	                    ...
	                    createFeatureInModerationMap(lonlat.lon,lonlat.lat,commune,pole,status,traiteparpole_poi,transmission_poi,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs,commentfinal_poi, modo,disp,lastDateModif,mail_poi,nbrComments);
...
function createFeatureInModerationMap(X,Y,commune,pole,status,traiteParPole,transmissionAuPole,reponseCollectivite,reponsePole,descriptionObs,propositionResolution,sousCategorie,rue,repereRue,prioriteParModerateur,observationTerrain,commentfinal_poi, estModere,affichageObservation,lastDateModif,emailObservateur,nbrComments) {
		...
		var moderationInterfaceWindow = new Ext.Window({
		    ...
		    bbar : {
		        ...
		        items : [
		            ...
		            new Ext.Toolbar({
		                height: 25,
		                items : ['Lat :', latitudeField, ' ', 'Long :', longitudeField,' ', 'Modération :', ModerationPOIField, ' | Affichage :', DisplayPOIField,' | Transmission au pole', TransmissionPolePOIField,' | Traité par pole:',TraitePolePOIField,'->', emailPOIField,sendMailButton,' ',printButtonForModerationWindow,' ',validButton, ' ', resetButton]
		            })
		        ]
		    }
		});
