...
var communePOIList = new Ext.data.JsonStore({
	...
	// checkbox pour afficher les observations qui n'ont pas de photos
	var displayObservationsWithoutPicture = 0;
	var checkboxDisplayObservationsWithoutPicture = new Ext.form.Checkbox({
	    boxLabel: T_ObsWithoutPicture,
	    checked: false,
	    listeners: {
	        'check': function () {
	        	console.log("checkboxDisplayObservationsWithoutPicture = "+this.getValue());
	            switch (this.getValue()) {
	            
	                case true:
	                	console.log("checkboxDisplayObservationsWithoutComment = true");
	                	displayObservationsWithoutPicture = 1;
	                    getMarker();
	                    console.log("checkboxDisplayObservationsWithoutPicture, apres getMarker");
	                    break;
	                case false:
	                	console.log("checkboxDisplayObservationsWithoutPicture = false");
	                	displayObservationsWithoutPicture = 0;
	                    getMarker();
	                    break;
	            }
	        }
	...
	checkboxDisplayObservationsWithoutPicture.on('afterrender', function(){
		  Ext.QuickTips.register({ target: checkboxDisplayObservationsWithoutPicture.getEl(), text: 'N\'afficher que les observations qui n\'ont pas de photo associée - se cumule avec les autres filtres.' });
		});  
...
function createFeatureInModerationMap(X,Y,commune,pole,status,traiteParPole,transmissionAuPole,reponseCollectivite,reponsePole,descriptionObs,propositionResolution,sousCategorie,rue,repereRue,prioriteParModerateur,observationTerrain,commentfinal_poi, estModere,affichageObservation,lastDateModif,emailObservateur,nbrComments,dateCreation) {
	...
	CommunePOIField.setValue(commune);
		...
		function fusionPoi (id1,id2){
			var message = '';
			if (!Number.isInteger(id1) || !Number.isInteger(id2) ){
				message= "Les numéros d'observations à fusionner doivent être des entiers";
			}else{
				Ext.Ajax.request({
					waitMsg: T_pleaseWait,
					url: 'lib/php/admin/database.php',
					...
					success: function(response) {
			            ...
			            case 1:
			            	Ext.MessageBox.show({
			                    title: T_careful,
			                    msg: "Les observations "+id1+" et "+id2+" ont été fusionnées. L'observation plus ancienne a reçu les observations et les votes de l'observation plus récente le cas échéant.L'observation plus récente doit être passée en priorité \"Doublon\" manuellement.",
			                    buttons: Ext.MessageBox.OK,
			                    icon: Ext.MessageBox.ERROR
			                }); 
			                break;
			            case 2:
			            	Ext.MessageBox.show({
			                    title: T_careful,
			                    msg: "Au moins une des observations "+id1+" et "+id2+" semble ne pas exister. Merci de vérifier les numéros.",
			                    buttons: Ext.MessageBox.OK,
			                    icon: Ext.MessageBox.ERROR
			                }); 
			                break;
			            case 3:
			            	Ext.MessageBox.show({
			                    title: T_careful,
			                    msg: "Les numéros des observations à fusionner doivent être différents. Merci de vérifier les numéros.",
			                    buttons: Ext.MessageBox.OK,
			                    icon: Ext.MessageBox.ERROR
			                }); 
			                break;
			            default:
			            	Ext.MessageBox.show({
			                    title: T_careful,
			                    msg: "Les observations "+id1+" et "+id2+" n'ont pas été fusionnées car un problème a été rencontré pendant le traitement.",
			                    buttons: Ext.MessageBox.OK,
			                    icon: Ext.MessageBox.INFO
			                });
			            	break;
