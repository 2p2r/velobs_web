...
function createFeaturePublicMapAndOpen(id,X,Y,icon,lib_subcategory,lib_status,desc,prop,iconCls,id,photo,num,rue,commune,repgt,cmt,date,lastDateModif,comments) {
    ...
    feat.attributes = {"id": id, "lib_subcategory": lib_subcategory, "lib_status": lib_status, "desc": desc, "prop": prop, "iconCls": iconCls, "photo": photo, "num": num, "rue": rue, "commune": commune, "repgt": repgt, "cmt": cmt, "date": date, "lastDateModif":lastDateModif, "comments": comments, "icon":icon};
	        ...
	        load: function() {
	            if (noIdParam != -1) {
	                if (idEdit != '' && idEdit != undefined && alreadyShowExpandWindowGetParameter == 0) {
	                    ...
	                    createFeature(lonlat.lon,lonlat.lat,commune,pole,status,traiteparpole_poi,transmission_poi,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs,commentfinal_poi, modo,disp,lastDateModif);
...
function createFeature(X,Y,com,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs,commentfinal_poi, modo,disp,lastDateModif) {
		...
		var expandWindow = new Ext.Window({
		    ...
		    bbar : {
		        ...
		        items : [
		            ...
		            new Ext.Toolbar({
		                height: 25,
		                items : ['Lat :', latitudeField, ' ', 'Long :', longitudeField,' ', 'Modération :', ModerationPOIField, 'Affichage :', DisplayPOIField,'Traité par pole:',TraitePolePOIField,'Transmission au pole', TransmissionPolePOIField,'->', ' ',validButton, ' ', resetButton]
		            })
		        ]
		    }
		});
