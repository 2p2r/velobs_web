	        ...
	        load: function() {
	            ...
	            if (noIdParam != -1) {
	                if (idEdit != '' && idEdit != undefined && alreadyShowExpandWindowGetParameter == 0) {
	                    ...
	                    moderationInterfaceWindow.setTitle(id + " " +this.getAt(indexEdit).json['lib_subcategory'] + ', '+ T_lastModificationDate + ' : ' + dateLastModif);
	                    ...
	                    if (tof != null) {
	                        Ext.getCmp('observationImagePanel').update('<img style="width:200px;cursor: pointer;" onclick="window.open(\'resources/pictures/'+tof+'\')" src="resources/pictures/'+tof+'" />');
	                    }else {
	                    ...
	                    createFeatureInModerationMap(lonlat.lon,lonlat.lat,commune,pole,status,traiteparpole_poi,transmission_poi,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs,commentfinal_poi, modo,disp,lastDateModif,email);
...
function createFeatureInModerationMap(X,Y,commune,pole,status,traiteParPole,transmissionAuPole,reponseCollectivite,reponsePole,descriptionObs,propositionResolution,sousCategorie,rue,repereRue,prioriteParModerateur,observationTerrain,commentfinal_poi, estModere,affichageObservation,lastDateModif,emailObservateur) {
