...
var resetPasswordButton = new Ext.grid.Column({
	header: T_resetPassword,
	dataIndex: 'latitude_poi', 
	width: 110,
	sortable: false,
	renderer: function(value, metaData, record, rowIndex, colIndex, store) {
        if (record.data.is_active_user){
        	return '<button>'+T_resetPasswordButton+'</button>';
        }
		return T_inactive_status_user;
	}
	
});
resetPasswordButton.on('click', function(columnIndex, grid, rowIndex, e) {
	var userId = grid.getStore().getAt(rowIndex).get('id_users');
	var userMail = grid.getStore().getAt(rowIndex).get('mail_users');
	var userLogin = grid.getStore().getAt(rowIndex).get('lib_users');
	...
	Ext.Ajax.request({   
		...
		success: function(response) {
			...
			Ext.MessageBox.alert(T_error, T_resetPasswordSucceeded);
		},
...
var UsersListingEditorGrid = new Ext.grid.EditorGridPanel({
...
UsersListingEditorGrid.on('afteredit', saveTheUser);
...
function saveTheUser(oGrid_event){
    ...
    Ext.Ajax.request({
        ...
        params: {
            ...
            is_active_user: oGrid_event.record.data.is_active_user
        },
        success: function(response) {
            var result = eval(response.responseText);
            switch (result) {
...
function confirmDeleteUsers() {
    if(UsersListingEditorGrid.selModel.getCount() == 1){
        Ext.MessageBox.confirm(T_confirm,T_deleteRecord, deleteUsers);
    } else if(UsersListingEditorGrid.selModel.getCount() > 1){
...
function deleteUsers(btn) {
    if (btn=='yes'){
        var selections = UsersListingEditorGrid.selModel.getSelections();
        var userz = [];
        for(i = 0; i< UsersListingEditorGrid.selModel.getCount(); i++){
            ...
            success: function(response) {
                ...
                switch (result) {
                    ...
                    case 3:
                        Ext.MessageBox.show({
                            title: T_pb,
                            msg: T_addedUserFailed + T_addedUserFailedInconsistency,
                            buttons: Ext.MessageBox.OK,
                            icon: Ext.MessageBox.INFO
                        });
                        break;
