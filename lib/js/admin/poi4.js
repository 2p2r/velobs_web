//composants devant être inaccessibles aux comptes des modérateurs associatifs
var arrayComponentsToDisable = ["StatusPOIField","RespComcomPOIField","RespPolePOIField"];
...
var expandWindow = new Ext.Window({
    ...
    bbar : {
        ...
        items : [
            ...
            new Ext.Toolbar({
                height: 75,
                items : ['[Public] Réponse de la collectivité:',RespComcomPOIField,'[Privé] Réponse de la collectivité:',RespPolePOIField,'[Public] Commentaire Vélo-Cité:',CommentFinalPOIField]
            }),
            new Ext.Toolbar({
                height: 25,
...
function createFeature(X,Y,com,pole,status,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,obs,comment,modo,disp) {
...
moderatePOIButton.on('click', function(columnIndex, grid, rowIndex, e) {
    ...
    if ((lat == '') && (lon == '')) {
        ...
        createFeature(lonlat.lon,lonlat.lat,commune,pole,status,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,obs,comment,modo,disp);
