...
function getMarker() {
	...
	if (tabcheckchild != ''  && setDefaultCenterMapDone == 1) {
		...
		var uri = 'lib/php/admin/getMarker.php?listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&dateNotModifiedSince='+dateNotModifiedSince+'&priority='+comboPrioritePOIMapItemMenu.getValue()+'&status='+StatusPOIFieldMapItemMenu.getValue()+'&displayObservationsToBeAnalyzedByComCom='+displayObservationsToBeAnalyzedByComCom+'&displayObservationsToBeAnalyzedByPole='+displayObservationsToBeAnalyzedByPole+'&commentToModerate='+displayObservationsWithCommentToModerate+'&nbSupportMinimum='+nbSupportMinimum+'&bounds=' + bounds+'&alreadyLoadedObservations='+alreadyLoadedObservations ;
