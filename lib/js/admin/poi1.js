...
var POIsDataStore = new Ext.data.Store({
    ...
    listeners: {
        load: function() {
        	console.info("POIsDataStore.load, noIdParam : "  + noIdParam);
            if (noIdParam != -1) {
...
var identifiantPOI = new Ext.grid.Column({
	...
	renderer: function(val) {
		console.info("identifiantPOI.renderer, "+T_displayRecordOnMap+" : "  + val);
...
identifiantPOI.on('contextmenu', function(columnIndex, grid, rowIndex, e) {
	...
	console.info("identifiantPOI.contextmenu, "+idPOI);
...
var moderatePOIButton = new Ext.grid.Column({
	...
	renderer: function(value, metaData, record, rowIndex, colIndex, store) {
		console.info("moderatePOIButton.renderer");
...
var expandWindow = new Ext.Window({
    ...
    bbar : {
        ...
        items : [
            ...
            new Ext.Toolbar({
                height: 25,
                items : ['Lat :', latitudeField, ' ', 'Long :', longitudeField,' ', 'Modération :', ModerationPOIField, 'Affichage :', DisplayPOIField,'->', moderatePhotos, ' ', moderateComments, ' ',validButton, ' ', resetButton]
            })
        ]
    }
});
