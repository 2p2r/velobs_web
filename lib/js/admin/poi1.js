	...
	if (activateReverseGeocoding){
		...
		Ext.Ajax.request({
			...
			success: function(response) {
				...
				RuePOIField.setValue(res[0]);
				CommunePOIField.setValue(res[1]);
				RuePOIField.enable();
				CommunePOIField.enable();
			},
			failure: function(response){
				var result = response.responseText;
...
var arrayItemsMenuTopAdminMap = ['Filtres : ',dateFieldModifiedSince,CancelDateFilterItem,'&',dateFieldNotModifiedSince,CancelDateNotModifiedFilterItem,'&', StatusPOIFieldMapItemMenu,CancelStatusFilterItem,'&',comboPrioritePOIMapItemMenu,CancelPrioriteFilterItem,'&',checkboxDisplayObservationsWithCommentToModerate,'&',checkboxDisplayObservationsWithoutPicture,'&',numberFieldNbSupport,CancelVoteFilterItem];
var arrayItemsMenuBottomAdminMap = [searchObservationField, checkBoxdisplayLayerLimitesGeographiquesCommunes,' ',checkBoxDisplaylayerLimitesGeographiquesPoles,' ',observation1ToFusionField,observation2ToFusionField,fusionButton,'->', comboALL];
	                        				...
	                        				if(PhotoPOIModifSuppForm.getForm().isValid()) {
	                        					PhotoPOIModifSuppForm.getForm().submit({
	                        						...
	                        						success: function(PhotoPOIModifSuppForm, o) {
	                            						...
	                            						Ext.getCmp('observationImagePanel').update('Cliquer sur l\'image pour l\'ouvrir en grand<br /><img style="width:250px;cursor: pointer;" onclick="window.open(\'resources/pictures/'+imageName+'\')" src="resources/pictures/'+imageName+'" />');
