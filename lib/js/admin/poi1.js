...
var photos = new Ext.grid.Column({
    ...
    renderer: function(value, metaData, record, rowIndex, colIndex, store) {
        if (value == 0) {
            metaData.css = 'nophoto';
            return '<span ext:qtip="'+T_simpleClickAddPhoto+'">'+value+' photo</span>';
        } else {
            metaData.css = 'photo';
            if (value == 1) {
...
var POIsColumnModel = new Ext.grid.ColumnModel(
	[identifiantPOI,{
        ...
        }*/, comments, photos/*,chkColumn_transmissionPOI*/
	]
);
...
//latitudePOI.on('dblclick', function(columnIndex, grid, rowIndex, e) {
	expandWindow.show();
	id = grid.getStore().getAt(rowIndex).get('id_poi');
    expandWindow.setTitle(T_record+' n°'+id);
...
/*latitudePOI.on('contextmenu', function(columnIndex, grid, rowIndex, e){
	e.preventDefault();
	var idPOI = grid.getStore().getAt(rowIndex).get('id_poi');
	Ext.Msg.confirm(T_confirm, T_sureModifGeo, function(btn, text) {
		if (btn == 'yes') {
			Ext.Ajax.request({   
				waitMsg: T_pleaseWait,
				url: 'lib/php/admin/database.php',
				...
				success: function(response) {            
					...
					switch (result) {
						case 1:
							...
							break;
						default:
							Ext.MessageBox.alert(T_careful,T_geoNotModify);
							break;
					}        
				},
				failure: function(response) {
...
/*longitudePOI.on('contextmenu', function(columnIndex, grid, rowIndex, e) {
	e.preventDefault();
	var idPOI = grid.getStore().getAt(rowIndex).get('id_poi');
	Ext.Msg.confirm(T_confirm, T_sureModifGeo, function(btn, text){
		if (btn == 'yes'){
...
var TabPanelRecord = new Ext.TabPanel({
	...
	items: [
		...
		/*CommuneListingEditorGrid,
		PoleListingEditorGrid,
		QuartierListingEditorGrid,*/
