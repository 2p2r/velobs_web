...
function createFeature(X,Y,com,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,obs) {
...
var  nbrComments = grid.getStore().getAt(rowIndex).get('num_comments');
	...
	if ((lat == '') && (lon == '')) {
        ...
        createFeature(lonlat.lon,lonlat.lat,commune,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,obs);
