        ...
        load: function() {
            if (noIdParam != -1) {
                if (idEdit != '' && idEdit != undefined && alreadyShowExpandWindowGetParameter == 0) {
                    ...
                    var transmission = this.getAt(indexEdit).json['transmission_poi'];
                    ...
                    createFeature(lonlat.lon,lonlat.lat,commune,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs,comment);
...
var latitudePOI = new Ext.grid.Column({
	...
	renderer: function(value, metaData, record, rowIndex, colIndex, store) {
		//return '<span ext:qtip="'+T_dblClickShowPos+'">'+value+'</span>';
...
var cyclemap = new OpenLayers.Layer.OSM("OpenStreetMap Cyclemap", "http://tile.opencyclemap.org/cycle/${z}/${x}/${y}.png",
	{'sphericalMercator': true, isBaseLayer:true}
);
...
expandMap.addLayers([mapquest, cyclemap, hikebike, mapnik]);
expandMap.setBaseLayer(mapquest);
...
var expandWindow = new Ext.Window({
    ...
    bbar : {
        ...
        items : [
            ...
            new Ext.Toolbar({
                height: 25,
                items : ['Traité par pole:',TraitePolePOIField,'Transmission au pole', TransmissionPolePOIField,'->', validButton, ' ', resetButton]
            })
        ]
    }
});
...
function createFeature(X,Y,com,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs) {
...
latitudePOI.on('click', function(columnIndex, grid, rowIndex, e) {
	...
	if ((lat == '') && (lon == '')) {
        ...
        createFeature(lonlat.lon,lonlat.lat,commune,pole,status,traite,transmission,reponsecomcom,reponsepole,desc,prop,subcat,rue,num,prio,obs);
...
function writePOICsv(){
	Ext.Ajax.request({
		url: 'lib/php/admin/writeCsv.php?type=poicomcom',
		success: function (response) {
