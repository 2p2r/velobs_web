...
var observation1ToFusionField = new Ext.form.NumberField({
	id: 'observation1ToFusionField',
	allowBlank: true,
	anchor : '99%',
	emptyText:'N° Observation 1 à fusionner',
	width:150,
});
observation1ToFusionField.on('afterrender', function(){
	  Ext.QuickTips.register({ target: this.getEl(), text: 'Numéro de la première observation à fusionner' });
	});
var observation2ToFusionField = new Ext.form.NumberField({
	id: 'observation2ToFusionField',
	allowBlank: true,
	anchor : '99%',
	emptyText:'N° Observation 2 à fusionner',
	width:150,
});
observation2ToFusionField.on('afterrender', function(){
	  Ext.QuickTips.register({ target: this.getEl(), text: 'Numéro de la seconde observation à fusionner' });
});

var fusionButton = new Ext.Button({
      	id:'fusionButton',
      	iconCls: '',
      	text:'Fusionner les observations',
      	handler: function() {
      	console.log("fusionPoi appelée");
      		if (observation1ToFusionField.getValue() == "" || observation2ToFusionField.getValue() == ""){
      			alert("Les 2 numéros d'observations à fusionner doivent être précisés.");
      		}else{
      			fusionPoi(observation1ToFusionField.getValue(),observation2ToFusionField.getValue());
      		}
