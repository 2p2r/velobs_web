...
function getMarker(listType,date,status) {
	...
	if (listType != '') {
		const _date = date === '' ? '2year' : date;
		var uri = 'lib/php/public/getMarker.php?listType='+listType+'&date='+_date+'&status='+status+'&done='+0;
		console.log(uri);
	...
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
            ...
            if (json.markers[i]['commentaires']!= null) {
               ...
               createFeaturePublicMap(id,lonlat.lon,lonlat.lat, json.markers[i].icon, json.markers[i].lib, json.markers[i].desc, json.markers[i].prop, json.markers[i].iconCls, json.markers[i].id, json.markers[i].photo, json.markers[i].num, json.markers[i].rue, json.markers[i].commune, json.markers[i].repgt, json.markers[i].cmt, json.markers[i].date, json.markers[i].lastdatemodif_poi, comments, json.markers[i].ref);
            } else {
                createFeaturePublicMap(id,lonlat.lon,lonlat.lat, json.markers[i].icon, json.markers[i].lib, json.markers[i].desc, json.markers[i].prop, json.markers[i].iconCls, json.markers[i].id, json.markers[i].photo, json.markers[i].num, json.markers[i].rue, json.markers[i].commune, json.markers[i].repgt, json.markers[i].cmt, json.markers[i].date, json.markers[i].lastdatemodif_poi, null, json.markers[i].ref);
            }
		}
	}
}
...
function createPopup(feature) {
	...
	htmlGeneral = "";
	htmlGeneral += "<b>Ref. fiche :</b> "+feature.attributes.ref;
...
function createFeaturePublicMap(id,X,Y,icon,lib,desc,prop,iconCls,id,photo,num,rue,commune,repgt,cmt,date,lastDateModif,comments, ref) {
    ...
    feat.attributes = {"id": id, "ref": ref, "lib": lib, "desc": desc, "prop": prop, "iconCls": iconCls, "photo": photo, "num": num, "rue": rue, "commune": commune, "repgt": repgt, "cmt": cmt, "date": date,"lastDateModif":lastDateModif, "comments": comments};
