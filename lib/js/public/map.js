...
function displayMarker(response) {
	...
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
            ...
            feat.attributes = {"id": id, "status":json.markers[i].status, "priority":json.markers[i].priority, "tooltip":id,"lib": json.markers[i].lib, "desc": json.markers[i].desc, "prop": json.markers[i].prop, "iconCls": json.markers[i].iconCls, "photo": json.markers[i].photo, "num": json.markers[i].num, "rue": json.markers[i].rue, "commune": json.markers[i].commune, "repgt": json.markers[i].repgt, "cmt": json.markers[i].cmt, "date": json.markers[i].date,"lastDateModif":json.markers[i].lastdatemodif_poi, "comments": comments,"icon":json.markers[i].icon};
            features.push(feat);
		}
		observationsVectorLayer.addFeatures(features);
		features = [];
...
//var observationsVectorLayer = new OpenLayers.Layer.Vector("marker", { styleMap: styleMap, visibility: true, displayInLayerSwitcher: false});
var observationsVectorLayer = new OpenLayers.Layer.Vector("marker", { styleMap: style, visibility: true, displayInLayerSwitcher: false,strategies: [new OpenLayers.Strategy.AnimatedCluster({
...
function showTooltip(event){ 
	...
	if (feature.cluster && feature.cluster.length < 10){
		...
		html+= 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.attributes.id+'">'+feature.attributes.id+'</a>, statut : '+feature.attributes.status+'<br />';
	}
	if (html!=''){
