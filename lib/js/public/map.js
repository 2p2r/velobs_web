...
function createPopup(feature) {
    ...
    htmlGeneral += '<br/><div style="width:100%;"><button type="button" onclick="addComment()">Ajouter un commentaire</button><button type="button" onclick="addPhoto('+idPoiComment+')">Ajouter une photo</button><a href="'+window.location.href.substring(0, window.location.href.lastIndexOf('/'))+'/admin.php?id='+feature.attributes.id+'"><img src="./resources/icon/silk/script_save.png" style="float:right;" /></a></div>';
...
treePanel.on('expandnode', function(node) {
	if (categories.indexOf(node.id) >= 0){
		if (node.isLeaf()){
			getCheckChild(this.getChecked('id'));
