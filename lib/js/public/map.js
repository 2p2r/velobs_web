...
var checkDisplayCommunesLayer = new Ext.form.Checkbox({
    boxLabel: "Limites",
    checked: false,
    listeners: {
        'check': function () {
...
var mapGeoExtPanel = new GeoExt.MapPanel({
	...
	tbar: new Ext.Toolbar({
		height: '20',
		cls: 'x-panel-header',
		items: [combodateALL, ' ',combostatusALL, '&nbsp;',checkdone, ' ', checkDisplayCommunesLayer, '->' ,comboALL]
	})
});
...
vectorsAdd.setZIndex( 1001 );
var polyboundRefresh = new OpenLayers.Strategy.Refresh({force: true});
var vectorsAdd2 = new OpenLayers.Layer.Vector("Limites communes", {strategies: [ polyboundRefresh ],reportError:true,styleMap:styleCommune, visibility: true, displayInLayerSwitcher: true,eventListeners:{
	 "featureadded": function(feature) {
		   console.log(feature);
		  }
		}});
vectorsAdd.setZIndex( 100);
publicMap.addLayer(vectorsAdd);
publicMap.addLayer(vectorsAdd2);

var selectControl = new OpenLayers.Control.SelectFeature([vectorsMap,vectorsAdd, vectorsAdd2]);
...
if (coords != ''){
	console.log("coords ="+coords.substr(0,11));
	for (var i in coords.substr(9,coords.length-11).split(',')) {
		if (i != 'remove'){
...
//	vectorsAdd2.addFeatures([feature_polygon]);
	vectorsAdd2.drawFeature(feature_polygon);
}
...
function displayCommunes(){
	...
	vectorsAdd2.display(true);
	if (communesLoaded == 0){
	var communes =[];
	//vectorsAdd2.removeAllFeatures();
	CommunePOIList.each(function(record){
		var polyCoords = [];
		var coords = record.get("geom");
		var commune = record.get("lib_commune");
 ...
 if(coords != ''){
		polyCoords = [];
		var positions = coords.substr(9,coords.length-11).split(',');
...
function hideCommunes(){
	console.log("hideCommunes");
	vectorsAdd2.display(false);
}
