...
function showTooltip(event){ 
	...
	if (feature.cluster && feature.cluster.length < 10){
		...
		for (var i = 0; i < feature.cluster.length; i++){
			html+= 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.cluster[i].attributes.id+'">'+feature.cluster[i].attributes.id+', statut donné par la collectivité : '+feature.attributes.status+'</a><br />';
		}
		title = T_directLinks;
	}else if (!feature.cluster){
		html+= 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.attributes.id+'">'+feature.attributes.id+'</a>, statut donné par la collectivité : '+feature.attributes.status+'<br />';
	}
	if (html!=''){
