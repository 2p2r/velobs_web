...
function displayMarker(response) {
	...
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
            ...
            feat.attributes = {"id": id, "status":json.markers[i].status, "priorite":json.markers[i].priorite, "tooltip":id,"lib": json.markers[i].lib, "desc": json.markers[i].desc, "prop": json.markers[i].prop, "iconCls": json.markers[i].iconCls, "photo": json.markers[i].photo, "num": json.markers[i].num, "rue": json.markers[i].rue, "commune": json.markers[i].commune, "repgt": json.markers[i].repgt, "cmt": json.markers[i].cmt, "date": json.markers[i].date,"lastDateModif":json.markers[i].lastdatemodif_poi, "comments": comments,"icon":json.markers[i].icon};
            features.push(feat);
		}
...
function showTooltip(event){ 
	...
	if (feature.cluster && feature.cluster.length < 10){
		...
		for (var i = 0; i < feature.cluster.length; i++){
			html+= 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.cluster[i].attributes.id+'">'+feature.cluster[i].attributes.id+'</a>, Priorité donnée par le modérateur : "'+feature.cluster[i].attributes.priorite+'", statut donné par la collectivité : "'+feature.cluster[i].attributes.status+'"<br />';
		}
		title = T_directLinks;
	}else if (!feature.cluster){
		html+= 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.attributes.id+'">'+feature.attributes.id+'</a>, Priorité donnée par le modérateur : "'+feature.attributes.priorite+'", Statut donné par la collectivité : "'+feature.attributes.status+'"<br />';
	}
	if (html!=''){
