...
function displayMarkerAndCenter(response) {
    ...
    if (json.markers != null) {
        ...
        if (json.markers[0]['commentaires']) {
            var comments = '<b>Commentaires :</b><br/>';
            for (var j=0; j<json.markers[0]['commentaires'].length;j++) {
                comments += ' - Ajouté le '+json.markers[0]['datecreation'][j] + ' : ';
                comments += ' Commentaire'+json.markers[0]['commentaires'][j] + ' : ';
                comments += ' Photo '+json.markers[0]['photos'][j] + ' : ';
                comments += '<br/><br/>';
            }
		...
		for (var i=0; i<json.markers.length; i++) {
            ...
            if (json.markers[i]['commentaires']!= null) {
                ...
                while(json.markers[i]['commentaires'][j] != null) {
                    comments += '<ul><li>Ajouté le '+json.markers[i]['datecreation'][j] + ' : </li>';
                ...
                console.info('displayMarker ' + comments);
...
//    }
	...
	popup = new GeoExt.Popup({
		title: feature.attributes.lib + ', date dernière modification : ' + feature.attributes.lastDateModif,
...
function createFeaturePublicMap(id,X,Y,icon,lib,desc,prop,iconCls,id,photo,num,rue,commune,repgt,cmt,date,lastDateModif,comments) {
    ...
    feat.attributes = {"id": id, "lib": templib, "desc": desc, "prop": prop, "iconCls": iconCls, "id": id, "photo": photo, "num": num, "rue": rue, "commune": commune, "repgt": repgt, "cmt": cmt, "date": date,"lastDateModif":lastDateModif, "comments": comments};
