...
//	OpenLayers.loadURL(uri, '', this, displayMarkerAndCenterInPublicMap);
	const url = "lib/php/public/getMarker.php";
	...
	xhr.onreadystatechange = () => {
        if (xhr.readyState === 4) {
        	...
        	displayMarkerAndCenterInPublicMap(xhr.response);
        }
      }
	xhr.send('id='+id);
		...
		if (json.markers[i]['commentaires']!=null) {
			...
			while(json.markers[i]['commentaires'][j] != null) {
				...
				if (json.markers[i]['photos'][j] != ""){
					comments += '<li>Photo associée <br /><a href="./resources/pictures/'+json.markers[i]['photos'][j]+'" target="_blank"><span ext:qtip="'+T_clickToOpenInNewTab+'"><img width="400" src="./resources/pictures/'+json.markers[i]['photos'][j]+'"/></span></a></li>';
				}
				comments += '</ul><hr />';
				j++;
			}
...
function displayMarker(response) {
	console.log("entering displayMarker " + response);
	document.getElementById("loader").style.display = "none";
	var json = eval('(' + response + ')');
	if (json.markers != null) {
		for (var i=0; i<json.markers.length; i++) {
			...
			feat.attributes = {"id": id, "color_status":json.markers[i].color_status,"lib_status":json.markers[i].lib_status, "lib_priorite":json.markers[i].lib_priorite, "lib_subcategory":json.markers[i].lib_subcategory, "tooltip":id, "desc": json.markers[i].desc, "prop": json.markers[i].prop, "iconCls": json.markers[i].iconCls, "photo": json.markers[i].photo, "num": json.markers[i].num, "rue": json.markers[i].rue, "commune": json.markers[i].commune, "repgt": json.markers[i].repgt,"reppole": json.markers[i].reppole, "cmt": json.markers[i].cmt, "date": json.markers[i].date,"lastDateModif":json.markers[i].lastdatemodif_poi, "comments": comments,"icon":json.markers[i].icon};
			features.push(feat);
		}
...
//	console.log("feature = %o", feature);
	...
	if (!feature.cluster){
		...
		if (featureToDisplay.attributes.reppole != '') {
			htmlGeneral += "<br/><b>"+T_reponsePole+" :</b> "+feature.attributes.reppole;
		} else {
			htmlGeneral += "<br/><b>"+T_reponsePole+" :</b> "+T_enAttenteReponsePole;
		}
...
function getMarker() {
	...
	if (tabcheckchild != '' && setDefaultCenterMapDone == 1) {
		...
		xhr.onreadystatechange = () => {
	        if (xhr.readyState === 4) {
	        	console.log(xhr.response);
	        	displayMarker(xhr.response);
	        }
	      }
		xhr.send('listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&status='+StatusPOIFieldMapItemMenu.getValue()+'&priorite='+comboPrioritePOIMapItemMenu.getValue()+'&nbSupportMinimum='+nbSupportMinimum+'&bounds=' + bounds+'&alreadyLoadedObservations='+alreadyLoadedObservations);
		...
		xhr2.onreadystatechange = () => {
	        if (xhr2.readyState === 4) {
	        	...
	        	displayNumberOfTotalRecords(xhr2.response);
	        }
	      }
		xhr2.send('listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&status='+StatusPOIFieldMapItemMenu.getValue()+'&priorite='+comboPrioritePOIMapItemMenu.getValue()+'&nbSupportMinimum='+nbSupportMinimum+'&getCount=1');
