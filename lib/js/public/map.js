...
function getMarker(listType,date,status) {
	...
	if (listType != '') {
		...
		var uri = 'lib/php/public/getMarker.php?listType='+listType+'&date='+date+'&status='+status+'&priorite='+comboPrioritePOIMapItemMenu.getValue();
...
var mapGeoExtPanel = new GeoExt.MapPanel({
	...
	tbar: new Ext.Toolbar({
		height: '20',
		cls: 'x-panel-header',
		items: [comboboxFilterObservationsByDate, ' ',comboboxFilterObservationsByStatus, '&nbsp;',comboPrioritePOIMapItemMenu, ' ', checkBoxdisplayLayerLimitesGeographiquesCommunes,' ',checkBoxDisplaylayerLimitesGeographiquesPoles, ' ',searchObservationField,searchObservationButton,'->' ,comboboxSelectMapLayer]
	})
});
