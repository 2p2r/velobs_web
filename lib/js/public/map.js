...
//checkbox affichage des limites geographiques sur la carte
var checkBoxdisplayLayerLimitesGeographiquesCommunes = new Ext.form.Checkbox({
    boxLabel: T_geographicalBounds,
    id : 'chkCommunes',
    checked: false,
    listeners: {
        'check': function () {
...
CommunePOIList.on('load', function(store, records, options){ 
	...
	CommunePOIList.each(function(record){
		...
		if(coords != ''){
			...
			layerLimitesGeographiquesCommunes.addFeatures([feature_polygon]);
...
PolePOIList.on('load', function(store, records, options){ 
	...
	PolePOIList.each(function(record){
		...
		if(coords && coords != ''){
			...
			layerLimitesGeographiquesPoles.addFeatures([feature_polygon]);
