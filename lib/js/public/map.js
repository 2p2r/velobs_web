...
var vectorsMap = new OpenLayers.Layer.Vector("marker", { styleMap: styleMap, visibility: true, displayInLayerSwitcher: false,strategies: [new OpenLayers.Strategy.Cluster({distance: 25})]});
...
function showTooltip(event){ 
	...
	tooltip = new Ext.ToolTip({ 
		html: 'Observation n° <a href="' + window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')) +'/index.php?id=' + feature.attributes.id+'">'+feature.attributes.id+'</a>', 
		closable: true,
		dismissDelay: 10000,
		width:200
	}); 
...
function createPublicPOI() {
	...
	//if the new observation marker is notcreated on the map, do not allow to create the observation
	if (feat == null){
		Ext.MessageBox.show({
			title: T_error,
