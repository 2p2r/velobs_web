...
function getMarker() {
	...
	if (tabcheckchild != '') {
		...
		var uri = 'lib/php/public/getMarker.php?listType='+tabcheckchild+'&dateLastModif='+dateLastModif+'&status='+status+'&priorite='+comboPrioritePOIMapItemMenu.getValue();
...
function displayMarkerAndCenterInPublicMap(response) {
	...
	if (json.markers != null) {
		...
		if (json.markers[i]['support']>0) {
			comments = '<b>Nombre de personnes soutenant cette fiche </b> : ' + json.markers[i]['support'];
		}else {
			comments = '<b>Nombre de personnes soutenant cette fiche </b> : Soyez le ou la première!!';
		}
...
//	console.log("feature = %o", feature);
	...
	if (!feature.cluster){
		...
		htmlGeneral += '<br/><div style="width:100%;"><button type="button" onclick="addComment()">Ajouter un commentaire</button> <button type="button" onclick="addSupport()">Soutenir cette demande</button><a href="'+window.location.href.substring(0, window.location.href.lastIndexOf('/'))+'/admin.php?id='+featureToDisplay.attributes.id+'"><img src="./resources/icon/silk/script_save.png" style="float:right;" /></a></div>';
	...
	items: [{
		...
		items:[{
			...
			items: [ {
					...
					xtype: 'panel',
					id: 'help',
					html: "Vous pouvez rajouter un nouveau commentaire",
	...
	items: [{
		...
		items:[{
			...
			items: [ mailSupportField, beInformedSupportField,{
					xtype: 'panel',
					id: 'helpSupport',
					html: "<p style=\"color:#555555\">Pour supporter cette observation, renseignez votre adresse e-mail. Celle-ci est nécessaire pour limiter les abus. Cette information ne sera communiquée en aucun cas à une tierce personne." +
							"Le nombre de personnes qui supporte cette observation est affichée et pourra permettre de prioriser les demandes d'aménagements</p>" +
...
var mapGeoExtPanel = new GeoExt.MapPanel({
	...
	tbar: new Ext.Toolbar({
		height: '20',
		cls: 'x-panel-header',
		items: [dateFieldModifiedSince, ' ',comboboxFilterObservationsByStatus, '&nbsp;',comboPrioritePOIMapItemMenu, ' ', checkBoxdisplayLayerLimitesGeographiquesCommunes,' ',checkBoxDisplaylayerLimitesGeographiquesPoles, ' ',searchObservationField,searchObservationButton,'->' ,comboboxSelectMapLayer]
	})
});
